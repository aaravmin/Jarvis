-- 0018_style_examples.sql, the learn-from-edits store. When the user edits something Jarvis generated
-- (an outreach email, an application field, a task) and keeps the edit, we record the before/after pair
-- here. Future generations of the same kind read recent pairs back so Jarvis matches the user's revealed
...
create table if not exists public.style_examples (
  ...
  ai_text     text not null,              -- what Jarvis produced
