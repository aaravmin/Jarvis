...
/**
 * The persistent full-height left navigation rail (md+). It's plain <Link>s, so navigation works
 * regardless of client hydration — the nav can never be "stuck closed". On small screens it's hidden
 * and the Topbar's hamburger <NavDrawer> takes over.
 */
export function Sidebar({ userEmail }: { userEmail?: string }) {
  const pathname = usePathname();

  return (
    <aside className="hidden md:flex md:w-60 md:shrink-0 md:flex-col border-r border-border bg-surface/80 backdrop-blur">
